  ...
  const generateAIStory = async () => {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          prompt: `Create a short children's story (ages 4-10) based on this idea: "${aiPrompt}". Return ONLY a JSON object with these fields (no markdown, no backticks): {"title": "Story title in English", "titleAr": "Story title in Arabic", "emoji": "One relevant emoji", "text": "The story text (150-200 words, simple language, magical and fun)", "words": ["5", "key", "words", "from", "story"]}`
        })
      });
